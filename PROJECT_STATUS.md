...
## Текущее состояние
- Статус: в разработке
- Последняя итерация: 2026-06-27 (Итерация 3 — загрузка данных)
- Что работает:
  ...
  - `/` — компактная таблица состояния БД по счётчикам; datepicker; быстрые периоды
  ...
  - **Итерация 2:** секция «Данные в базе» на `/` — табы Визиты/Хиты, GET `/api/table-preview`, пагинация, горизонтальный скролл
  - **Итерация 2:** CJM — padding узлов `6px 20px`; главная `/` всегда на графе (SQL + post-filter)
  - **Итерация 3:** `/` — порядок секций: состояние БД → параметры → прогресс → данные (скрыты по умолчанию)
  - **Итерация 3:** кнопка «Посмотреть данные в БД» / «Скрыть данные» под таблицей состояния
  ...
  - **Итерация 3:** Flatpickr для выбора дат; быстрые периоды синхронизированы с календарём
...
- `/api/table-preview` — постраничный просмотр `raw_metrika.visits|hits`, сортировка по `date DESC`, опциональный фильтр `counter_id`; колонки без данных на текущей странице скрываются
