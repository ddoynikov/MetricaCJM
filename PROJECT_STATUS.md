...
- Что работает:
  ...
  - **Итерация 3:** `/` — порядок секций: состояние БД → параметры → прогресс → результат
  ...
  - **Итерация 5:** popover пропусков — даты ДД.ММ.ГГГГ, кнопка «Догрузить пропуски» → range-picker + scroll к форме
  - **Итерация 5:** попап БД — `counter_user_id_hash` вторая колонка, клик → `/cjm?user_hash=…`, сортировка по заголовкам
  - **Итерация 5:** CJM — автозаполнение поиска из URL-параметра `user_hash`
  - **Итерация 5:** Flatpickr — полная тема под design tokens
  - **Итерация 6:** отступы между карточками (`section-card`), компактный dropdown счётчика (`#counterId`, max-width 480px)
  - **Итерация 6:** «Догрузить пропуски» — исправлен setDate через `parseApiDate`, обновление `#dateRangeText`
  - **Итерация 6:** «Превью визитов/хитов» открывает попап БД с нужным табом и фильтром счётчика
  - **Итерация 6:** кнопка «Пересчитать CJM» в секции результата → POST `/api/cjm/refresh`
  - **Итерация 6:** попап БД — фильтр служебных пустых колонок (`["0"]`, `[""]` и т.п.)
  - **Итерация 7:** popover пропусков — max-height, счётчик дней, позиционирование в viewport
  - **Итерация 7:** попап БД — whitelist ключевых колонок (date, utm_*, start_url и др.)
  - **Итерация 7:** «Перейти к CJM» → `/cjm?counter_id=…`; CJM автовыбор счётчика из URL
  - **Итерация 7:** CJM — min_transitions: слайдер + числовое поле (sync)
  - **Итерация 7:** «Пересчитать CJM» в результате — POST `/api/cjm/refresh?counter_id=…`
  - **Итерация 7:** `/api/cjm/refresh` — опциональный `counter_id`, пересчёт только для счётчика
